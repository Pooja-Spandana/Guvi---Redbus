...
if web == 'Home':
    ...
    st.markdown('''#### :blue[Developed By:] Pooja Spandana''')
    ...
    def get_states():
        query = "SELECT DISTINCT State_name FROM Red_bus"
    ...
    def get_routes_by_state(state):
        query = "SELECT DISTINCT Route_name FROM Red_bus WHERE State_name = %s"
    ...
    def get_price_range(route):
        query = "SELECT MIN(Price), MAX(Price) FROM Red_bus WHERE Route_name = %s"
    ...
    def get_filtered_data(selected_state, selected_route, selected_bus_types, selected_price_ranges, selected_ratings):
        query = "SELECT * FROM Red_bus WHERE State_name = %s AND Route_name = %s"
